...
class show_nice_gedcom:
    ...
    def __init__(self, widget, window):
        ...
        # set the individual cells "text" attribute to matching column -
        # retrieve text from that column in individual treestore
        column = 0
        ...
        # set the family cells "text" attribute to matching column - retrieve
        # text from that column in family treestore
        column = 0
        ...
        # set the note cells "text" attribute to matching column - retrieve
        # text from that column in note treestore
        column = 0
        ...
        # set the submitter cells "text" attribute to matching column -
        # retrieve text from that column in submitter treestore
        column = 0
        ...
        # set the repository cells "text" attribute to matching column -
        # retrieve text from that column in repository treestore
        column = 0
        ...
        # set the source cells "text" attribute to matching column - retrieve
        # text from that column in source treestore
        column = 0
        ...
        # set the object cells "text" attribute to matching column - retrieve
        # text from that column in object treestore
        column = 0
        ...
        # set the submission cells "text" attribute to matching column -
        # retrieve text from that column in submission treestore
        column = 0
        ...
        # set the custom cells "text" attribute to matching column - retrieve
        # text from that column in custom treestore
        column = 0
        ...
        actiongroup.add_actions([('Convert Dot Graph',
                                  gtk.STOCK_OPEN,
                                  '_Open Dot...',
                                  None,
                                  None,
                                  self.convert_dot_graph)])

        actiongroup.add_actions([('Last Name Frequency',
                                  None,
                                  '_Last Name Frequency...',
                                  None,
                                  None,
                                  self.last_name_frequency_report)])
    ...
    def individual_delete_cb(self, cell, path, model):
        model[path][
            self.individual_delete_posn] = not model[path][
            self.individual_delete_posn]
        iter = model.get_iter(path)
        result = model.remove(iter)

        return

    def raw_gedcom_delete_cb(
            self, cell, path, raw_gedcom_treestore, input_treestore, input_iter):
        raw_gedcom_treestore[path][
            self.raw_gedcom_delete_posn] = not raw_gedcom_treestore[path][
            self.raw_gedcom_delete_posn]
    ...
    def family_cursor_changed(self, treeview):
        ...
        if colname in ("Husband\nGiven\nNames", "Husband\nLast\nName"):
            ...
            if family_husband_pointer:
                self.set_cursor(
                    family_husband_pointer,
                    self.individual_treeview)
        if colname in ("Wife\nGiven\nNames", "Wife\nLast\nName"):
        ...
        while self.generic_iter:
            ...
            if self.temp_individual_sex == "M":
                individual = self.temp_individual_pointer.split("@")[1] + ' [label="' + self.temp_individual_given_names + ' ' + self.temp_individual_last_name + \
                    ...
                    self.temp_individual_death_date + '",color=lightblue]' + '\n'
            else:
                individual = self.temp_individual_pointer.split("@")[1] + ' [label="' + self.temp_individual_given_names + ' ' + self.temp_individual_last_name + \
                    '\\n' + 'B: ' + self.temp_individual_birth_date + '\\n' + 'D: ' + \
                    self.temp_individual_death_date + '",color=lightpink]' + '\n'
    ...
    def save_gedcom(self, action):
        ...
        file_save = gtk.FileChooserDialog(
            title="Save GEDCOM File",
            action=gtk.FILE_CHOOSER_ACTION_SAVE,
            buttons=(
                gtk.STOCK_CANCEL,
                gtk.RESPONSE_CANCEL,
    ...
    def open_gedcom(self, action):
        ...
        if file_name:
            ...
            for e in g.element_list():
                if e.tag() == "FAM":
                    ...
                    self.family_dict[
                        e.pointer()] = (
                        husband, wife, [
                            date, place], children, e.get_individual())
            ...
            for e in g.element_list():
                if e.level() == 0 and e.tag() not in ("INDI", "FAM", "HEAD",
                                                      "TRLR", "NOTE", "SUBM", "REPO", "SOUR", "OBJE", "SUBN"):
                    self.custom_dict[e.pointer()] = (e.get_individual())

            pd = sorted(self.individual_dict.keys())

            for p in pd:
                self.get_individual_info(p)

                piter = self.individual_treestore.append(
                    None,
                    [
                        self.individual_raw_gedcom,
                        "INDIVIDUAL",
                        p,
                        self.individual_given_names,
                        self.individual_last_name,
                        self.individual_sex,
                        self.individual_birth_date,
                        self.individual_birth_year,
                        self.individual_birth_month,
                        self.individual_birth_day,
                        self.individual_death_date,
            ...
            for f in fd:
                ...
                piter = self.family_treestore.append(None,
                                                     [self.family_raw_gedcom,
                                                      "FAMILY",
                                                      f,
                                                      self.family_husband_pointer,
                                                      self.family_husband_given_names,
                                                      self.family_husband_last_name,
                                                      self.family_wife_pointer,
                                                      self.family_wife_given_names,
                                                      self.family_wife_last_name,
                                                      self.family_marriage_date,
            ...
            self.on_show_note(
                ...
                ": Objects\n" +
                "%4.0f" %
                self.total_submission_count +
                ": Submissions\n" +
                "%4.0f" %
                self.total_custom_count +
                ": Custom",
                "OK")
    ...
    def save_dot_graph(self, action):
        ...
        if not individual_iter:
            self.on_show_note(
                self.window,
                "ERROR: Must select individual to generate parent DOT graph for!",
                "OK")
            return
        ...
        if everyone_dot_flag:
            ...
            if parents_dot_flag:
                ...
                self.on_show_note(
                    self.window,
                    "Dot file successfully generated for parents of " +
                    pointer,
                    "OK")
            ...
            if children_dot_flag:
                ...
                self.on_show_note(
                    self.window,
                    "Dot file successfully generated for children of " +
                    pointer,
                    "OK")
    ...
    def traverse_children(self, individual_pointer,
                          living_dates_flag, spouses_dot_flag, direction_flag=None):
        self.find_individual(individual_pointer, self.individual_treeview)
        individual_sex = self.temp_individual_sex

        if living_dates_flag == False and (self.temp_individual_death_date == "" and (
                self.temp_individual_birth_year != "" and datetime.date.today().year - int(self.temp_individual_birth_year) < 100)):
            if self.temp_individual_sex == "M":
        ...
        for f in individual_family_pointers:
            if f:
                ...
                if (spouses_dot_flag == True) or (spouses_dot_flag ==
                                                  False and self.traverse_level < traverse_level_max):
                    family = self.temp_family_pointer.split(
                        "@")[1] + ' [shape=box, ranksep=0.1, orientation=270.0, label="' + 'M: ' + self.temp_family_marriage_date + '", color=palegreen]' + '\n'
                    self.dot_file.write(family)
                ...
                family_children_pointers = string.split(
                    self.temp_family_children_pointers, ",")
                if len(family_children_pointers) > 1 and self.traverse_level < (
                        traverse_level_max - 1):
                    # if len(family_children_pointers) > 1:
                    child_list = ""
                    prev_j = ""
                    child_arrow = ""
    ...
    def traverse_parents(self, individual_pointer, living_dates_flag, siblings_flag=None,
                         spouses_dot_flag=False, children_dot_flag=False, direction_flag=None):
        self.find_individual(individual_pointer, self.individual_treeview)
        ...
        if individual_pointer not in self.parent_hit:
            if living_dates_flag == False and (self.temp_individual_death_date == "" and (
                    self.temp_individual_birth_year != "" and datetime.date.today().year - int(self.temp_individual_birth_year) < 100)):
                if self.temp_individual_sex == "M":
                    individual = individual_pointer.split(
        ...
        for p in individual_parent_pointers:
            if p in self.family_dict:
                ...
                if father_pointer:
                    if father_pointer not in self.parent_hit:
                        self.traverse_level += 1
                        if direction_flag is None:
                            father_link = p.split(
                        ...
                        self.traverse_level -= 1
                if mother_pointer:
                    if mother_pointer not in self.parent_hit:
                        self.traverse_level += 1
                        if direction_flag is None:
                            mother_link = p.split(
    ...
    def get_next_individual_pointer(self):
        ...
        while self.next_individual_pointer in self.individual_dict:
            self.next_individual_pointer_number = self.next_individual_pointer_number + 1
            self.next_individual_pointer = "@I" + \
                str(self.next_individual_pointer_number).zfill(4) + "@"
    ...
    def get_next_family_pointer(self):
        ...
        while self.next_family_pointer in self.family_dict:
            self.next_family_pointer_number = self.next_family_pointer_number + 1
            self.next_family_pointer = "@F" + \
                str(self.next_family_pointer_number).zfill(4) + "@"
    ...
    def get_next_note_pointer(self):
        ...
        while self.next_note_pointer in self.note_dict:
            self.next_note_pointer_number = self.next_note_pointer_number + 1
            self.next_note_pointer = "@N" + \
                str(self.next_note_pointer_number).zfill(4) + "@"
    ...
    def show_valid_tags(self, widget):
        ...
        vt = sorted(self.valid_tags.keys())
    ...
    def individual_on_row_expanded(self, store, iter, path):
        ...
        if store.get_value(child, 0) is None:
            ...
            if relation_style == "ParentChild":
                if individual_pointer not in self.individual_dict:
                    print "ERROR: individual pointer does NOT exist in GEDCOM file!", individual_pointer
                else:
                    individual_family_pointers = string.split(store.get_value(
                        iter, self.individual_family_pointers_posn), ",")
                    for f in individual_family_pointers:
                        if f in self.family_dict:
                            self.find_family(f, self.family_treeview)
                            ...
                            if individual_pointer == husband_pointer:
                                ...
                                if w:
                                    ...
                                    piter = self.individual_treestore.append(
                                        iter,
                                        [
                                            self.temp_individual_raw_gedcom,
                                            "WIFE",
                                            w,
                                            self.temp_individual_given_names,
                                            self.temp_individual_last_name,
                                            self.temp_individual_sex,
                                            self.temp_individual_birth_date,
                                            self.temp_individual_birth_year,
                                            self.temp_individual_birth_month,
                                            self.temp_individual_birth_day,
                                            self.temp_individual_death_date,
                                            self.temp_individual_death_year,
                                            self.temp_individual_death_month,
                                            self.temp_individual_death_day,
                                            self.temp_individual_family_pointers,
                                            self.temp_individual_parent_pointers])
                                    family_children_pointers = string.split(
                                        self.temp_family_children_pointers, ",")
                                    for c in family_children_pointers:
                                        if c in self.individual_dict:
                                            self.find_individual(
                                                c, self.individual_treeview)
                                            citer = self.individual_treestore.append(
                                                piter,
                                                [
                                                    self.temp_individual_raw_gedcom,
                                                    "CHILD",
            ...
            if relation_style == "ChildParent":
                if individual_pointer not in self.individual_dict:
                    ...
                    individual_parent_pointers = string.split(store.get_value(
                        iter, self.individual_parent_pointers_posn), ",")[0]
                    if individual_parent_pointers in self.family_dict:
                        self.find_family(
    ...
    def get_individual_info(self, individual_pointer):
        ...
        if self.individual_family_pointers and self.individual_family_pointers[
                -1] == ",":
    ...
    def get_family_info(self, family_pointer):
        ...
        if family_pointer not in self.family_dict:
        ...
        if self.family_husband_pointer in self.individual_dict:
        ...
        if self.family_wife_pointer in self.individual_dict:
        ...
        if self.family_children_pointers and self.family_children_pointers[
                -1] == ",":
    ...
    def add_child(self, widget):
        ...
        if prev_individual_iter:
            ...
            if type not in ("HUSBAND", "WIFE", "NEWHUSBAND", "NEWWIFE"):
                ...
                self.on_show_note(
                    self.window,
                    "ERROR: Not positioned on proper parent row!\n" +
                    individual_pointer,
                    "OK")
                return
        else:
        ...
        if not fp:
            ...
            self.on_show_note(
                self.window,
                "ERROR: family pointer does NOT exist in GEDCOM file!\n" +
                fp,
                "OK")
            return

        individual_family_pointers = string.split(
            individual_treestore.get_value(
                individual_iter, self.individual_family_pointers_posn), ",")
        for family_pointer in individual_family_pointers:
            if family_pointer in self.family_dict:
                self.find_family(family_pointer, self.family_treeview)
                if (self.temp_family_husband_pointer == individual_pointer) and (
                        self.temp_family_wife_pointer == prev_individual_pointer):
        ...
        if individual_family_pointers in self.family_dict:
            ...
            self.family_treestore.set_value(
                self.generic_iter,
                self.family_children_pointers_posn,
                new_children_pointers)
        else:
            print "No family to attach child too!"
            self.on_show_note(
                self.window,
        ...
        # dare, beware that you are adding a child as a new person -- not one
        # that might already have his own info/relationships

        new_child_iter = self.individual_treestore.insert_after(
            individual_iter,
            None,
            [
                "",
        ...
        self.save_raw_gedcom(
            raw_gedcom_treestore,
            self.individual_treestore,
            new_child_iter)
    ...
    def add_note(self, widget):
        ...
        new_note_iter = self.note_treestore.prepend(
            ...
            [
                "",
                "NEWNOTE",
                self.next_note_pointer,
                "Comment line starts here...and continues on next line with no breaks.\nFresh comment line starts here..."])
    ...
    def save_raw_gedcom(self, raw_gedcom_treestore,
                        input_treestore, input_iter):
        new_values = ""
    ...
    def add_parent(self, parent_type):
        ...
        self.individual_treestore.set_value(
            individual_iter,
            self.individual_raw_gedcom_posn,
            new_raw_gedcom)
        ...
        new_parent_iter = self.individual_treestore.insert_before(
            None,
            None,
            [
                "",
        ...
        add_spouse_iter = self.family_treestore.prepend(
            None,
            [
                family_raw_gedcom,
                "NEWFAMILY",
    ...
    def delete_individual(self, widget):
        ...
        # if deleted individual is a husband/wife, remove HUSB/WIFE tag from
        # each family raw gedcom and also set individual's name/pointer to
        # empty in each family
        if individual_family_pointers:
            family_pointers = string.split(individual_family_pointers, ",")
            for family in family_pointers:
                if family in self.family_dict:
                    self.find_family(family, self.family_treeview)
                    self.delete_raw_gedcom_line(
                        self.temp_family_raw_gedcom, [
                            "HUSB", "WIFE"], individual_pointer)
                    self.family_treestore.set_value(
        ...
        # if deleted individual is a child in a family, remove CHIL tag in
        # family raw gedcom and also remove child pointer from family children
        # pointer list
        if individual_parent_pointers in self.family_dict:
        ...
        result = self.treerow_search_and_delete(
            self.individual_treestore,
            self.individual_treestore.iter_children(None),
            self.match_func,
            (self.individual_pointer_posn,
             self.temp_individual_pointer))
    ...
    def replace_raw_gedcom_line(
            self, input_gedcom, input_value, check_tag, check_value):
        raw_gedcom = input_gedcom
        self.new_raw_gedcom = ""
        for raw_gedcom_line in raw_gedcom.split("\n"):
            line_element = raw_gedcom_line.split(" ", 2)
            ...
            # if tag in check_tag and values == check_value:
            if tag in check_tag:
                if (values == check_value) or (check_value is None):
    ...
    def add_family(self, widget):
        ...
        self.save_raw_gedcom(
            raw_gedcom_treestore,
            self.family_treestore,
            family_treestore_iter)
    ...
    def add_spouse(self, widget):
        ...
        if not individual_iter:
            self.on_show_note(
                self.window,
                "ERROR: Must select spouse to attach to!",
                "OK")
            return
        ...
        if individual_sex == "M":
            ...
            new_spouse_iter = self.individual_treestore.insert_after(
                individual_iter,
                None,
                [
                    "",
        ...
        new_family_iter = self.family_treestore.prepend(
            None,
            [
                family_raw_gedcom,
                "NEWFAMILY",
        ...
        if individual_sex == "M":
            self.family_treestore.set_value(
                new_family_iter,
                self.family_husband_given_names_posn,
                individual_given_names)
            self.family_treestore.set_value(
                new_family_iter,
                self.family_husband_last_name_posn,
                individual_last_name)
            self.family_treestore.set_value(
                new_family_iter,
                self.family_husband_pointer_posn,
                individual_pointer)
            self.family_treestore.set_value(
                new_family_iter,
                self.family_wife_pointer_posn,
                self.next_individual_pointer)
        else:
            self.family_treestore.set_value(
                new_family_iter,
                self.family_wife_given_names_posn,
                individual_given_names)
            self.family_treestore.set_value(
                new_family_iter,
                self.family_wife_last_name_posn,
                individual_last_name)
            self.family_treestore.set_value(
                new_family_iter,
                self.family_wife_pointer_posn,
    ...
    def copy_individual(self, output_iter, input_iter):
        ...
        self.individual_treestore.set_value(
            output_iter,
            self.individual_raw_gedcom_posn,
            temp_individual_raw_gedcom)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_given_names_posn,
            temp_individual_given_names)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_last_name_posn,
            temp_individual_last_name)
        self.individual_treestore.set_value(
            output_iter, self.individual_sex_posn, temp_individual_sex)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_birth_date_posn,
            temp_individual_birth_date)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_birth_year_posn,
            temp_individual_birth_year)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_birth_month_posn,
            temp_individual_birth_month)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_birth_day_posn,
            temp_individual_birth_day)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_death_date_posn,
            temp_individual_death_date)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_death_year_posn,
            temp_individual_death_year)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_death_month_posn,
            temp_individual_death_month)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_death_day_posn,
            temp_individual_death_day)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_family_pointers_posn,
            temp_individual_family_pointers)
        self.individual_treestore.set_value(
            output_iter,
            self.individual_parent_pointers_posn,
            temp_individual_parent_pointers)
    ...
    def raw_gedcom_edit(self, raw_gedcom_treestore,
                        input_treestore, input_iter, window):
        global raw_gedcom_treeview
        ...
        level_cell.connect(
            'edited',
            self.raw_gedcom_level_edited_cb,
            raw_gedcom_treestore,
            input_treestore,
            input_iter)
        ...
        pointer_cell.connect(
            'edited',
            self.raw_gedcom_pointer_edited_cb,
            raw_gedcom_treestore,
            input_treestore,
            input_iter)
        ...
        values_cell.connect(
            'edited',
            self.raw_gedcom_values_edited_cb,
            raw_gedcom_treestore,
            input_treestore,
            input_iter)
        raw_gedcom_treeview.connect(
            'cursor-changed',
            self.raw_gedcom_cursor_changed_cb)
        ...
        actiongroup2.add_actions([('Insert Raw Gedcom Row Before', None, '_Insert Raw Gedcom Row Before', None, None, self.raw_gedcom_row_insert_before),
                                  ('Insert Raw Gedcom Row After',
                                   None,
                                   '_Insert Raw Gedcom Row After',
                                   None,
                                   None,
                                   self.raw_gedcom_row_insert_after),
    ...
    def raw_gedcom_cursor_changed_cb(self, raw_gedcom_treeview):
        ...
        if colname in ("Values") and value[:1] == "@":
            if tag in ("INDI", "HUSB", "WIFE", "CHIL"):
                ...
                if individual_pointer in self.individual_dict:
                    self.set_cursor(
                        individual_pointer,
                        self.individual_treeview)
            if tag[:3] == "FAM":
            ...
            if tag == "NOTE":
                note_pointer = value
                if note_pointer in self.note_dict:
                    self.set_cursor(note_pointer, self.note_treeview)
            if tag == "SUBM":
                submitter_pointer = value
                if submitter_pointer in self.submitter_dict:
                    self.set_cursor(submitter_pointer, self.submitter_treeview)
            if tag == "SOUR":
                source_pointer = value
                if source_pointer in self.source_dict:
                    self.set_cursor(source_pointer, self.source_treeview)
            if tag == "REPO":
                respository_pointer = value
                if repository_pointer in self.repository_dict:
                    self.set_cursor(
                        repository_pointer,
                        self.repository_treeview)
            if tag == "OBJE":
                object_pointer = value
                if object_pointer in self.object_dict:
                    self.set_cursor(object_pointer, self.object_treeview)
            if tag == "SUBN":
                submission_pointer = value
                if submission_pointer in self.submission_dict:
                    self.set_cursor(
                        submission_pointer,
                        self.submission_treeview)

    def raw_gedcom_level_edited_cb(
            self, cell, path, new_text, raw_gedcom_treestore, input_treestore, input_iter):
        ...
        try:
            ...
            self.on_show_note(
                self.window,
                "ERROR: level value must be numeric integer",
                "OK")
            return
        ...
        if int(level) != 0 and pointer != " ":
            self.on_show_note(
                self.window,
                "ERROR: Pointer value only valid for level 0!\n" +
                pointer,
                "OK")
            return
    ...
    def raw_gedcom_pointer_edited_cb(
            self, cell, path, new_text, raw_gedcom_treestore, input_treestore, input_iter):
        raw_gedcom_treestore[path][
        ...
        if input_treestore == self.individual_treestore:
            if pointer not in self.individual_dict and pointer != "":
                print "ERROR: individual pointer does NOT exist in GEDCOM file!", pointer
                self.on_show_note(
                    self.window,
                    "ERROR: individual pointer does NOT exist in GEDCOM file!\n" +
                    pointer,
                    "OK")
                return

        if input_treestore == self.family_treestore:
            if pointer not in self.family_dict and pointer != "":
    ...
    def raw_gedcom_values_edited_cb(
            self, cell, path, new_text, raw_gedcom_treestore, input_treestore, input_iter):
        """
        ...
        if tag == "NAME":
            ...
            if len(name) < 2:
                ...
                individual_sex = input_treestore.get_value(
                    input_iter, self.individual_sex_posn)
                individual_family_pointers = string.split(input_treestore.get_value(
                    input_iter, self.individual_family_pointers_posn), ",")
                # dare, now does family pointers get updated?
                for family_pointer in individual_family_pointers:
                    if family_pointer in self.family_dict:
        ...
        if tag == "SEX":
            ...
            if sex not in ('M', 'F'):
                self.on_show_note(
                    self.window,
                    "ERROR: SEX tag must be either 'M' or 'F'",
                    "OK")
                return
            else:
                input_treestore.set_value(
        ...
        if tag == "HUSB":
            ...
            if pointer not in self.individual_dict and pointer != "":
                ...
                # dare, do you REPLACE existing indi family info, or APPEND new
                # stuff to it? and what about the original spouse?
        ...
        if tag == "WIFE":
            ...
            if pointer not in self.individual_dict and pointer != "":
                ...
                # set remove family pointer from original wife individual
                # family pointer list and also remove FAMS entry from raw
                # gedcom
                self.find_individual(old_values, self.individual_treeview)
                self.delete_pointer_list(
                    self.temp_individual_family_pointers, family_pointer)
                ...
                # append current family to family pointer of new wife
                # individual and also append FAMS entry to raw gedcom
                self.find_individual(new_text, self.individual_treeview)
                self.append_pointer_list(
                    self.temp_individual_family_pointers, family_pointer)
        ...
        if tag == "NOTE":
            pointer = new_text
            if pointer not in self.note_dict and pointer != "":
                print "ERROR: note pointer does NOT exist in GEDCOM file!", pointer
                self.on_show_note(
                    self.window,
                    "ERROR: note pointer does NOT exist in GEDCOM file!\n" +
                    pointer,
                    "OK")
                return
        if tag in ["CONC", "CONT"]:
        ...
        if input_treestore == self.individual_treestore:
            ...
            match_iter = self.treerow_search_and_copy(
                self.individual_treestore,
                self.individual_treestore.iter_children(None),
                self.match_func,
                (self.individual_pointer_posn,
...
def get_ini_data(self):
    ...
    try:
        ...
        self.on_show_note(
            self.window,
            "ERROR: mgedcom.ini file missing! Creating minimal version.",
            "OK")
        ...
        try:
            ...
            write_ini_data(
                ...
                'parents_dot_flag',
                parents_dot_flag)
            print "Did not find parents_dot_flag in ini file so adding it"
        else:
...
class main_window:
    ...
    def __init__(self):
        ...
        if not os.path.exists(os.path.expanduser("~/.mgedcom")):
            os.system('mkdir -p ~/.mgedcom')
            os.system(
                'cp -p /usr/lib/mgedcom/home/user/.mgedcom/fake.* ~/.mgedcom')
            print "Copying missing fake gedcom files into mgedcom directory..."
            os.system(
                'cp -p /usr/lib/mgedcom/home/user/.mgedcom/mgedcom.ini ~/.mgedcom')
            print "Copying missing mgedcom.ini into mgedcom directory..."

        # if not os.path.exists("~/.mgedcom/mgedcom.ini"):
        if not os.path.exists(os.path.expanduser("~/.mgedcom/mgedcom.ini")):
            os.system(
                'cp -p /usr/lib/mgedcom/home/user/.mgedcom/mgedcom.ini ~/.mgedcom')
            print "Copying missing mgedcom.ini into mgedcom directory..."
